...
public class SNIDApp {
    ...
    /**
     * Constructor for SNIDApp class <br>
     * 
    ...
    /**
     * Method to add existing Citizens from the file This method needs to be
     * modularized as it violates the Single responsibility principle
     * 
            ...
            while (database.hasNext()) {
                ...
                if (commentLine == true) {
                    ...
                    if (currentLine.length <= 7) {
                        Citizen citizen = new Citizen(gender, yearOfBirth, firstName, middleName, lastName);
                        citizen.setId(id);
                        records.add(citizen);
                    ...
                    Citizen citizen = new Citizen(id, firstName, middleName, lastName, gender, yearOfBirth, lifeStatus,
                            addressOne, addressTwo, addressThree, addressFour, addressFive, motherId, fatherId);
                    records.add(citizen);
    ...
    /**
     * Method to register the birth of a Citizen <br>
    ...
    /**
     * method to register death of a person <br>
     * a CivicDoc is created theat contains the information about the persons death
     * <br>
     * retreives the Citizen information form records by the use of thier id <br>
    ...
    /**
     * Method to register two Citizens' marriage. <br>
     * The marriage between two individuals is recorded in the papers arraylist in
     * the citizen class <br>
    ...
    /**
     * Private method to search for the Id of a citizen using a binary search <br>
     * the list is sorted then the id is compared againsit others in the list this
     * works by dividing the list into equal halfs and compares the wanted id with
     * the other in the middle of list each iteration
     * 
     * @author Mario Anckle
     * @author Jason Gayle
     * @param id A String representing the id of the Citizen being searched
     * @return An integer representing the index in the arraylist with the desired
     *         Citizen
     */
    ...
    /**
     * Method to add parentData to a citizen <br>
     * finds the index of the citizen and their parent(s) id and stores them as obj
     * <br>
     * set the parent of the citizen if the id is found and updates the citezen
     * records, else thorws an IndexOutOfBoundsException
     * 
    ...
    /**
     * Method to get the Citizen's father. <br>
     * get father's position using the idSearch method and returns the father's
     * details if his ID exsits
     * 
    ...
    public String getFather(String id) {
        ...
        if (father != null) {
            ...
            return fatherId + "," + fatherName.getFirstName() + "," + fatherName.getMiddleName() + ","
                    + fatherName.getLastName();
        }
        return "";
    }

    /**
     * Method to get the Citizen's mother get mother's position using the idSearch
     * method and returns the mother's details if his ID exsits
     * 
     * @param id The id of the Citizen
    ...
    public String getMother(String id) {
        ...
        if (mother != null) {
            ...
            return motherId + "," + motherName.getFirstName() + "," + motherName.getMiddleName() + ","
                    + motherName.getLastName();
        }
        return "";
    }

    /**
     * Method to update the address of the citizen <br>
     * This works alongside the private idSearch method to find the Citizen then
     * update their address based on their id <br>
     * 
    ...
    /**
     * Method to return a mailing label of a citizen <br>
    ...
    public String mailingLabel(String id) {
        ...
        return personName.getLastName().toUpperCase() + "," + personName.getFirstName() + " "
                + personName.getMiddleName() + "\n" + address;
    }

    public String displayCitizenDetails(String id){
        Citizen person = records.get(idSearch(id));
        String address;
        Name personName = person.getNameObj();
        
        // Sets the life status to Alive if its A, and Dead if its not
        String lifeStatus = person.getLifeStatus()=='A'?"Alive":"Dead";
        String gender = Character.toString(person.getGender());
        String birthYear = Integer.toString(person.getYOB());
        // Sets the marriage details as the toString for the marriageDoc if its not null and an empty string otherwise
        String marriageDetails = person.getMarriageDoc() != null?person.getMarriageDoc().toString():"Not Married";
        try{
            address = person.getAddress().toString();
        }catch(NullPointerException e){
            System.out.println("An address had not been set");
        ...
        String headerHr = "___________________________";
        String header = "            Citizen Details        ";
        String line1 = String.format("ID no: %s",person.getId());
        String line2 = String.format("Name: %s %s %s",personName.getFirstName(),personName.getMiddleName(),personName.getLastName());
        String line3 = String.format("Gender: %s",gender);
        String line4 = String.format("Year of Birth: %s",birthYear);
        String line5 = String.format("Life Status: %s", lifeStatus);
        String line6 = String.format("Address Details:\n%s",address);
        String line7 = String.format("Marriage Details:\n%s", marriageDetails);
        return  "\n" + header + "\n" + headerHr + "\n\n" + line1 + "\n" + 
                line2 + "\n" + line3 + "\n" + line4 + "\n" +line5 + "\n\n" + line6 + 
                "\n\n" + line7;
    ...
    public String search(String id) {
        ...
        if (index < 0) {
            ...
            if (records.get(index).equals(null)) {
                ...
                Citizen person = records.get(idSearch(id));
                Name personName = person.getNameObj();
                return id + "," + person.getGender() + "," + personName.getFirstName() + ","
    ...
    public String[] search(String firstName, String lastName) {
        ...
        if (index < 0) {
            ...
            Citizen person = records.get(index);
            Name personName = person.getNameObj();
            String[] citiArr = { person.getId(), Character.toString(person.getGender()), personName.getFirstName(),
                    personName.getMiddleName(), personName.getLastName() };
            return citiArr;
        }
    }
        ...
        for (int index = 0; index < records.size(); index++) {
            ...
            try {
                if (biodata.toString().equals(tag + value)) {
                    Name personName = person.getNameObj();
                    String[] citiArr = { person.getId(), Character.toString(person.getGender()),
                            personName.getFirstName(), personName.getMiddleName(), personName.getLastName() };
                    return citiArr;
                }
            } catch (NullPointerException e) {
    ...
    /**
     * private method used to split address <br>
     * takes in the address param of and splits it using the split method and
     * returns it in the toString method form the Address class <br>
     ...
     * 
     * @param address
     * @return An address string containg all parts of the address splited into
     *         parts
     */
     ...
     * 
     * @param writeList The arrayList where the other objects to be written to the
     *                  file are being set
    ...
    /**
     * private method to wirte citizen information to a list The primary method for
     * building the list to be written to the file
     * 
     * @param citizen
